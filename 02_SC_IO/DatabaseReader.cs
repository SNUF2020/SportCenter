...
namespace SC
{
    class DatabaseReader : IDisposable
    {
        ...
        public enum GpxObjectType { None, Attributes, Athletdata, AthletLogData };
        ...
        //public List<activity> _database;
            ...
            while (Reader_.Read())
            {
                switch (Reader_.NodeType)
                {
                    case XmlNodeType.Element:
                        if (Reader_.Name != "SportCenter_DataBase") throw new FormatException(Reader_.Name); // Testing if this file is a SC database
            ...
            while (Reader_.Read())
            {
                switch (Reader_.NodeType)
                {
                    ...
                    case XmlNodeType.EndElement:
                        if (Reader_.Name != "SportCenter_DataBase" & Reader_.Name != "AthleteLog") throw new FormatException(Reader_.Name);
                    ...
                    case XmlNodeType.Element:
                        ...
                        switch (Reader_.Name)
                        {
                            ...
                            case "Calories":
                                ...
                                break;
                            case "Speed":
                                activitydata.speedattributes = ReadSpeedAttributes();
                                break;
                            case "Info":
                                activitydata.infoattributes = ReadInfoAttributes();
                                break;
                            case "Laps":
            ...
            while (Reader_.Read())
            {
                switch (Reader_.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        switch (Reader_.Name)
                        {
                            ...
                            case "pt":
                                ...
                                //MessageBox.Show(trackdata_list[trackdata_list.Count - 1].lat.ToString());
                                break;
                            default:
                                SkipElement();
                                break;
                    ...
                    case XmlNodeType.CDATA:
                        //string track_string = Unzip(Convert.FromBase64String(_reader.Value));
                        //File.WriteAllText("test", track_string.ToString(), System.Text.Encoding.Default);

                        using (ZipReader Reader_zip = new ZipReader(DatabaseHelpers.Unzip(Convert.FromBase64String(Reader_.Value))))
                        {

                            try
                            {
                                while (Reader_zip.Read())
                                {
                                    switch (Reader_zip.ObjectType)
                                    {
                                        case ZipReader.ZipObjectType.Zip:
                                            trackdata_list = Reader_zip.trackdata_list_zip;
                                            break;
                                    }
                                }
                            }
                            ...
                            {
                                Console.WriteLine(ex.Message + "Zip_Load");
            ...
            while (Reader_.Read())
            {
                switch (Reader_.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        {
                            case "Distance":
                                data.TotalMeters = Convert.ToDouble(Reader_.GetAttribute("TotalMeters"), CultureInfo.InvariantCulture);
                                break;
                            case "Calories":
                            ...
                            case "Intensity":
                                ...
                                break;
                            case "Cadence":
                                data.Cadence = Convert.ToDouble(Reader_.GetAttribute("Cad"));
                                break;
                            case "Trigger":
                                data.Trigger = Convert.ToDouble(Reader_.GetAttribute("Trig"));
                                break;
                            case "Speed":
                                data.Speed_max = Convert.ToDouble(Reader_.GetAttribute("Speed_max"), CultureInfo.InvariantCulture);
                                break;
                            case "Start":
        ...
        private InfoAttributes ReadInfoAttributes()
        {
            InfoAttributes infoattributes = new InfoAttributes();
            ...
            while (Reader_.MoveToNextAttribute())
            {
                ...
                {
                    case "Notes":
                        infoattributes.Notes = Reader_.Value;
                        break;
                    case "Name":
                        infoattributes.Name = Reader_.Value;
                        break;
                }
            }
        ...
        private SpeedAttributes ReadSpeedAttributes()
        {
            SpeedAttributes attributes = new SpeedAttributes();
            ...
            while (Reader_.MoveToNextAttribute())
            {
                ...
                {
                    case "MaxSpeed":
                        attributes.MaxSpeed = Convert.ToDouble(Reader_.Value, CultureInfo.InvariantCulture);
                        break;
                }
            }
        ...
        private CaloriesAttributes ReadCaloriesAttributes()
        {
            ...
            while (Reader_.MoveToNextAttribute())
            {
                ...
                {
                    case "TotalCal":
                        attributes.TotalCal = Convert.ToDouble(Reader_.Value, CultureInfo.InvariantCulture);
                        break;
                }
            }
